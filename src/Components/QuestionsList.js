...
function QuestionsList({ unanswered, answered, showQuestions }) {
    ...
    <div className="questions-list-container">
      {showQuestions === "unanswered" ? (
        unansweredArray.length > 0 ? (
          unansweredArray.map(q => <Poll key={q.id} id={q.id} />)
        ) : (
          <div className="loader">
            ...
            <div className="no-unanswered">
              There aren't unanswered questions.
            </div>
          </div>
        )
      ) : answeredArray.length > 0 ? (
        answeredArray.map(q => <Poll key={q.id} id={q.id} />)
      ) : (
        <div className="loader">
          <Loader
            type="Puff"
